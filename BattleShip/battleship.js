...
function gameboard(){
    ...
    this.receiveAttack = function receiveAttack(attatcker, location, name){
        for(let i = 0; i < this.ships.length; i++){
            ...
            document.getElementById(`${attatcker}${location}`).style = "background-color:green";
            return;
    }
...
function gameplayLoop(input){
    if (playerBoard.ships.length < 5){
        if (!playerPlacements.includes(input) && !playerPlacements.includes(input+1) && !playerPlacements.includes(input+2)){
            ...
            playerPlacements.push(input , input+1, input+2, input+3);
            document.getElementById(`playerboard${input}`).style = "background-color:pink";
            document.getElementById(`playerboard${input+1}`).style = "background-color:pink";
            document.getElementById(`playerboard${input+2}`).style = "background-color:pink";
            document.getElementById(`playerboard${input+3}`).style = "background-color:pink";

        }
        let computerInput = null;

        while(computerInput === null || computerPlacements.includes(computerInput) || computerPlacements.includes(computerInput+1) || computerPlacements.includes(computerInput+2) || computerPlacements.includes(computerInput+3) || outOfBounds.includes(computerInput)){
